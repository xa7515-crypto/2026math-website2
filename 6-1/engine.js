...
(function () {
  ...
  // 將教材文字中的斜線分數統一排成上下分數線；避免題目與解答出現 3/5。
  function textbookMath(value) {
    return String(value ?? '')
      .replace(/(\d+)又(\d+)\/(\d+)/g, (_, w, n, d) => `\\(${w}\\dfrac{${n}}{${d}}\\)`)
      .replace(/(\d+)\/(\d+)/g, (_, n, d) => `\\(\\dfrac{${n}}{${d}}\\)`);
  }
  ...
  function openExampleModal(s) {
    ...
    document.getElementById('zoomBody').innerHTML =
      `<div class="zoom-q">${textbookMath(s.example.q)}</div>
       <div class="zoom-sol" id="zoomSolBox">
         ${s.example.steps ? `<ol>${s.example.steps.map(t => `<li>${textbookMath(t)}</li>`).join('')}</ol>` : ''}
         ${s.example.ans ? `<div class="zoom-ans">答：${textbookMath(s.example.ans)}</div>` : ''}
       </div>`;
    showZoom(s, '範例');
  ...
  function render() {
    ...
    if (s.type === 'divider') {
      ...
      if (s.points && s.points.length) {
        html += `<ul class="points">${s.points.map(p => `<li>${textbookMath(p)}</li>`).join('')}</ul>`;
      }
      if (s.example) {
        html += `<div class="example">
          <div class="ex-head">範例<button class="ex-zoom" title="放大成整頁，方便用畫筆講解">🔍 放大</button></div>
          <div class="ex-q">${textbookMath(s.example.q)}</div>
          <button class="ex-toggle">顯示解答</button>
          <div class="ex-sol">
            ${s.example.steps ? `<ol>${s.example.steps.map(t => `<li>${textbookMath(t)}</li>`).join('')}</ol>` : ''}
            ${s.example.ans ? `<div class="ex-ans">答：${textbookMath(s.example.ans)}</div>` : ''}
          </div>
        </div>`;
      }
      ...
      if (s.caption) {
        ...
        cap.innerHTML = textbookMath(s.caption);
        vis.appendChild(cap);
      }
