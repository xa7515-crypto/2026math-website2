...
(function () {
  ...
  function buildTOC() {
    ...
    DECK.forEach(chap => {
      ...
      flat.forEach((s, i) => {
        ...
        if (listedSections.has(s.sec)) return;
        listedSections.add(s.sec);
        const b = document.createElement('button');
        b.className = 'toc-item';
        b.dataset.i = i;
        b.dataset.sec = s.sec;
        b.innerHTML = `<span class="ti-sec">${s.sec}</span>${s.secName}`;
  ...
  function markTOC() {
    ...
    tocEl.querySelectorAll('.toc-item').forEach(b => {
      b.classList.toggle('active', current.type === 'slide' && b.dataset.sec === current.sec);
    });
  }
